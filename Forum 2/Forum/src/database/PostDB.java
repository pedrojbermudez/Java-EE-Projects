...
public class PostDB {
   ...
   */
  public boolean editPost(int postId, int forumId, String post) {
    boolean done = false;
    PreparedStatement stm = null;
    // Creating a new connection
    db = new DBConnection();
  ...
  public boolean deletePost(int postId, int userId) {
    ...
    try {
      stm = conn.prepareStatement("update " + Constant.POST_TABLE + " set "
          + Constant.POST_POST_FIELD_NAME + "=?, "
          + Constant.POST_MODIFICATION_DATE_FIELD_NAME + "=?, "
          + Constant.POST_DELETED_FIELD_NAME + "=? where "
          + Constant.POST_POST_ID_FIELD_NAME + "=?");
      if (userId == -1)
  ...
  public ArrayList<String[]> getPosts(int threadId, int index, int elements) {
    ...
    try {
      ...
      if (rs != null) {
        while (rs.next()) {
          String[] tmp = { Integer.toString(rs.getInt("post_id")),
              Integer.toString(rs.getInt("post_user_id")), rs.getString("post"),
              rs.getString("creat_date"), rs.getString("mod_date"),
              rs.getString("user_name"), rs.getString("picture") };
          posts.add(tmp);
        }
      }
    } catch (SQLException e) {
  ...
  public String[] getPost(int postId) {
    ...
    try {
      ...
      stm = conn
          .prepareStatement("select " + Constant.POST_USER_ID_FIELD_NAME + ", "
      ...
      if (rs.next()) {
        ...
        post[0] = rs.getString(Constant.POST_POST_FIELD_NAME);
        post[1] = Integer.toString(rs.getInt(Constant.POST_USER_ID_FIELD_NAME));
      }
    } catch (SQLException e) {
