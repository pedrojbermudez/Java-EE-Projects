...
<%
	...
	if (session != null && session.getAttribute("id") != null
		...
		// TODO make changes in new forum select, moderator user.
		...
		if (request.getParameter("fid") != null && request.getParameter("fid").matches("^\\d+$")) {
			...
			webContent.append("<form action=\"edit-forum/\" method=\"POST\">"
					+ "<input type=\"hidden\" name=\"forum_id\" value=\"" + request.getParameter("fid") + "\">"
					+ "Name: <input type=\"text\" value=\"" + fm[0] + "\" required name=\"forum_name\""
					+ " placeholder=\"Introduce a name for the forum\"><br>"
					+ "Description: <textarea type=\"text\" " + "name=\"forum_description\">" + fm[1]
					+ "</textarea><br>" + "Moderators: "
					+ user.getModUsers(Integer.parseInt(request.getParameter("fid"))) + "<br>Category: "
					+ category.getCategoryWEBSelect("forum_category_id",
