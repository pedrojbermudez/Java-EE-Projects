...
public class ThreadDB {
   ...
   *         user_name(3)
   */
  public ArrayList<ThreadF> getThreadList(int forumId, int index,
      int totalElements) {
    ArrayList<ThreadF> list = new ArrayList<>();
    stm = null;
    String sql = "";
    try {
      ...
      while (rs.next()) {
        ...
        // ArrayList<String[]>
        list.add(new ThreadF(rs.getInt(Constant.THREAD_THREAD_ID_FIELD_NAME),
            forumId, rs.getString(Constant.THREAD_NAME_FIELD_NAME),
            rs.getInt(Constant.THREAD_USER_ID_FIELD_NAME),
  ...
  public ArrayList<Integer> getThreadIdList(int forumId) {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        list.add(rs.getInt(Constant.THREAD_THREAD_ID_FIELD_NAME));
      }
    } catch (SQLException e) {
      System.err.println("Error in getThreadIdList():");
      System.err.println("Message => " + e.getMessage());
      System.err.println("SQL state => " + e.getSQLState());
      System.err.println("SQL sentence => " + sql);
      System.err.println("1? => " + forumId);
  ...
  public int getTotalThreads(int forumId) {
    ...
    try {
      ...
      System.err.println("Error in getTotalThreads():");
      System.err.println("Message => " + e.getMessage());
      System.err.println("SQL state => " + e.getSQLState());
      System.err.println("SQL sentence => " + sql);
      System.err.println("1? => " + forumId);
  ...
  public ThreadF getThread(int threadId) {
    ...
    try {
      ...
      if (rs.next()) {
        ...
        thread = new ThreadF(threadId,
            rs.getInt(Constant.THREAD_FORUM_ID_FIELD_NAME),
            rs.getString(Constant.THREAD_NAME_FIELD_NAME),
            rs.getInt(Constant.THREAD_USER_ID_FIELD_NAME));
      }
    } catch (SQLException e) {
      System.err.println("Error in getThread():");
      System.err.println("Message => " + e.getMessage());
      System.err.println("SQL state => " + e.getSQLState());
      System.err.println("SQL sentence => " + sql);
      System.err.println("1? => " + threadId);
  ...
  public String getThreadTitle(int threadId) {
    ...
    try {
      ...
      System.err.println("Error in getThreadTitle():");
      System.err.println("Message => " + e.getMessage());
      System.err.println("SQL state => " + e.getSQLState());
      System.err.println("SQL sentence => " + sql);
      System.err.println("1? => " + threadId);
   ...
   */
  public ArrayList<ThreadF> get30Threads() {
    ArrayList<ThreadF> threads = new ArrayList<>();
    stm = null;
    String sql = "";
    long start, end;
    try {
      ...
      while (rs.next()) {
        ...
        forumName = rs2.next() ? rs2.getString(Constant.FORUM_NAME_FIELD_NAME)
            : "";
        // Saving the data into a temporal Array String and add it to the list
        // as ThreadF object
        threads.add(new ThreadF(threadId, forumId, threadName, userId, userName,
            forumName));
      }
