...
public class MessageDB {
    ...
    try {
      ...
      stm = conn
          .prepareStatement("select " + Constant.MESSAGE_MESSAGE_FIELD_NAME
              ...
              + Constant.MESSAGE_RECEIVER_FIELD_NAME + "=? order by "
              + Constant.MESSAGE_MESSAGE_ID_FIELD_NAME + "asc limit ?,?");
      stm.setInt(1, sender);
      stm.setInt(2, receiver);
      stm.setInt(3, index-1);
      stm.setInt(4, totalElements);
  ...
  public ArrayList<String[]> getMessageList(int userId) {
    PreparedStatement stm = null;
    Connection conn = null;
    DBConnection db = null;
    ArrayList<String[]> messages = new ArrayList<>();
    try {
      ...
      stm = conn
          .prepareStatement("select " + Constant.MESSAGE_MESSAGE_FIELD_NAME
              + ", " + Constant.MESSAGE_CREATION_DATE_FIELD_NAME + " from "
              + Constant.MESSAGE_TABLE + " where "
              + Constant.MESSAGE_SENDER_FIELD_NAME + "=? or "
              + Constant.MESSAGE_RECEIVER_FIELD_NAME + "=? group by "
              + Constant.MESSAGE_SENDER_FIELD_NAME + ", "
              + Constant.MESSAGE_RECEIVER_FIELD_NAME + "");
      stm.setInt(1, userId);
      stm.setInt(2, userId);
